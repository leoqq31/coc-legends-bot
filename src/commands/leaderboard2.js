...
const { getGuildClans, getWeeklyLegendLeaderboard, setLegendBoardL2 } = require('../database/queries');
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Show Legend 2 weekly leaderboard (auto-updates)'),
  ...
  async execute(interaction) {
    ...
    for (const clan of clans) {
      const entries = getWeeklyLegendLeaderboard.all(yearWeek, clan.clan_tag, 'L2');
      allEntries.push(...entries);
    }
    allEntries.sort((a, b) => (b.end_trophies || b.trophies || 0) - (a.end_trophies || a.trophies || 0));

    const guildName = interaction.guild?.name || 'Server';
    const embed = weeklyLegendEmbed(guildName, allEntries, yearWeek, 'II');
    embed.setFooter({ text: 'Auto-updates every 1 min' });
    embed.setTimestamp();

    const reply = await interaction.reply({ embeds: [embed], fetchReply: true });
    setLegendBoardL2.run(interaction.guildId, interaction.channelId, reply.id);
  },
};
